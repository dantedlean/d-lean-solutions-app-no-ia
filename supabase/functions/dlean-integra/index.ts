...
Deno.serve(async (req: Request) => {
  ...
  try {
        ...
        JSON.stringify({
          ...
          maxiprod: {
            ...
            message: isRegistered
              ? 'Empresa já cadastrada no CRM do Maxiprod.'
              : 'Nova empresa! Cadastrada no Maxiprod e visita registrada com sucesso.',
          },
        }),
        {
    ...
    if (action === 'create_task') {
      ...
      if (actualQuoteId) {
        const { error: insertError } = await supabase.from('quote_engineering_status').upsert(
          {
        ...
        if (insertError) {
          ...
          console.log(
            `[Email Mock] Enviando e-mail para engenharia@dlean.com.br sobre o orçamento ${orderNumber}...`,
          )
