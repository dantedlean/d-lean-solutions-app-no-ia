  ...
  try {
    ...
    if (action === 'generate_image') {
      ...
      if (prompt) {
        ...
        const colorMatch = prompt.match(/RAL\s+(.*?)\s+powder/i) || prompt.match(/Cor:\s*Pintura na cor RAL\s*([^\n]+)/i);
        if (colorMatch) {
    ...
    if (action === 'assist_briefing') {
      ...
      const dim = eqData.dimensoes_externas || eqData.dimensoes || equipment?.dimensoes || 'Não especificadas';
      const niveis = eqData.nNiveis || equipment?.nNiveis || eqData.niveis?.length || 1;
      const cargaRaw = eqData.carga_tampo || eqData.cargaTotal || eqData.carga || eqData.peso || equipment?.cargaTotal || equipment?.carga || '0';
      const espessura = eqData.espessuraTubo || eqData.espessura || equipment?.espessuraTubo || 'padrão';
      const type = eqData.equipamento || equipment?.type || 'Equipamento';
      
      const carga = parseFloat(String(cargaRaw).replace(',', '.').replace(/[^0-9.]/g, '')) || 0;
      ...
      if (!suggestion.includes('ALERTA')) {
        suggestion += "Conclusão: Dimensões e configurações iniciais estão consistentes e tecnicamente viáveis. Pode seguir para a engenharia.";
      }
    ...
    if (action === 'generate_justification') {
      ...
      const methodText = equipment?.method === 'lean' ? 'estruturas modulares (Lean Pipe)' : 'estruturas soldadas';
